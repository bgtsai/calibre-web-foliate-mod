...
(function () {
    ...
    //   高度後的可用空間。
    // v0.9.4（這版）：v0.9.3 的樣式覆蓋數值上確認生效了（#viewer computed
    //   height 956px，跟預期完全吻合），但畫面看起來仍然沒變。追查後發現
    //   是執行順序的問題：原本 hideOldUI()（含這條覆蓋規則）放在 view.open()
    //   之後才呼叫，foliate-js 在 open() 當下已經用「舊的、還沒被我們改過」
    //   的 #viewer 尺寸量好版面、算完排版了，我們後來才改變容器尺寸，它不會
    //   自動重新量一次。這版把 hideOldUI() 挪到最前面，搶在 view.open() 之前
    //   執行，確保 foliate-js 從一開始量到的就是正確的最終尺寸。
    // v0.9.5（這版）：兩項架構性調整。(1) hideOldUI() 原本用「手動列清單」
    //   的方式指定要隱藏哪些舊 Calibre-Web 元件，這種做法本質上會漏東西
    //   （這次就漏了 #divider 這個分隔線元件才被發現）。改成反過來：#main
    //   底下除了我們自己的 #viewer，其餘一律隱藏，不用管裡面實際上有什麼、
    //   叫什麼名字，徹底接管。(2) 把散落的全域變數（__cwfmShadowMap／
    //   __cwfmBlobMap／__cwfmCreateTOCView／__cwfmCreateMenu）統一整合進
    //   window.__cwfm 底下（.shadowMap／.blobMap／.createTOCView／
    //   .createMenu），跟原本就有的 .view／.book／.bookId／.settings 收在
    //   同一個命名空間。這次改名過程中，額外抓到一個真正的 bug：
    //   paginator.js／fixed-layout.js 裡讀取 blob 對照表那段程式碼還在用
    //   舊名字 window.__cwfmBlobMap，沒同步改到，會導致查表機制悄悄失效、
    //   退回會被 CSP 擋掉的舊行為——推送前這輪驗證抓到並一併修正了。
    // v0.9.6（這版）：三項修正。(1) 查證 W3C CSS Grid 規格官方文件，確認
    //   margin=0 貼頂不是 bug：規格明定「非彈性欄位（我們的內容欄，上限
    //   1440px）會先長到自己的最大值，1fr 彈性欄位才能拿到剩餘空間」，
    //   視窗可用高度遠小於 1440px，內容欄會整個吃光，上下 1fr 欄位自然都是
    //   0——這是瀏覽器規格定義的行為，margin=0 在數學上就是等於「不保證任何
    //   留白」，沒有辦法同時要求置中。改成滑桿最小值設為 4（不再允許選到
    //   0），從規格上保證上下兩欄位算式相同、拿到的空間一定相等。(2) 之前
    //   答應要做但沒有真正實作的「拆成上下/左右兩個獨立像素滑桿、改細成
    //   1px 一階」，這版終於補上：上下留白對應 margin 屬性（本來就是
    //   px）；左右留白對應 gap 屬性（底層吃百分比），即時把使用者調的像素
    //   值換算成對應百分比，並在視窗尺寸改變時重新換算，避免實際像素值
    //   隨視窗縮放漂移。(3) 有些書封面圖片本身沒設定保持長寬比，容器尺寸
    //   一變動就整張拉伸變形，加上 object-fit: contain 強制保持原始比例。
